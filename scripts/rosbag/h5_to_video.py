...
class EventReaderAbstract:
    def __init__(self, filepath: Path):
        ...
        assert filepath.name.endswith('.h5') or filepath.name.endswith('.hdf5'), "Provided file is not an HDF5 file"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--delta_time_ms', '-dt_ms', type=float, default=50.0,
                        help='Time window (in milliseconds) to summarize events for visualization')
    ...
    assert video_filepath.parent.is_dir(), f"Directory {video_filepath.parent} does not exist"
